...
public class PropertiesFileReader {
	...
	private static final Logger LOGGER = Logger.getLogger(PropertiesFileReader.class.getSimpleName());
	 ...
	 */
	public PropertiesFileReader(final RequestContext context, final String propertiesFileName) {
	 ...
	 * @return value set the param value as properties file.
	 */
	public static String getPropertiesValue(final String propertiesFileName, final String key) {
		String value = "";
		if (key != null && !key.equals("")) {
	 ...
	 */
	public static String getPropertiesValue(final RequestContext context, final String propertiesFileName,
			final String key) {
		String value = "";
		if (key != null && !key.equals("")) {
			value = propertiesFileLoader(context, propertiesFileName).getProperty(key);
		}
		return value;
	}

	private static Properties propertiesFileLoader(final String propertiesFileName) {
		LOGGER.info("Loading Properties File.");
		Properties propFile = new Properties();
		if (propertiesFileName != null && !propertiesFileName.equals("")) {
			InputStream propertyFileAsStream = PropertiesFileReader.class.getResourceAsStream(propertiesFileName);
	...
	private static Properties propertiesFileLoader(final RequestContext context, final String propertiesFileName) {
		...
		if (propertiesFileName != null && !propertiesFileName.equals("")) {
			...
			InputStream inputStream = fileDal.getStream(root + "/iw/config/properties/" + propertiesFileName);
			if (inputStream != null) {
				try {
