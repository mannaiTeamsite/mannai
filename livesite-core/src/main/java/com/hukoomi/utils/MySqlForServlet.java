...
public class MySqlForServlet {
	 ...
	 */
	private final Logger logger = Logger.getLogger(MySqlForServlet.class);
	/**
	...
	public void releaseConnection(Connection con, Statement stmt, ResultSet rs) {
		...
		if (con != null) {
			try {
				...
				logger.error("MySqlForServlet : releaseConnection() : connection : ", e);
			}
		}
		if (stmt != null) {
			try {
				...
				logger.error("MySqlForServlet : releaseConnection() : statement : ", e);
			}
		}
		if (rs != null) {
			try {
				...
				logger.error("MySqlForServlet : releaseConnection() : resultset : ", e);
			}
		}
	}
}
