...
public class MySql {
	...
	private void loadProperties(final RequestContext context) {
		PropertiesFileReader propertyFileReader = new PropertiesFileReader(context, "dbconfig.properties");
	...
	public void releaseConnection(Connection con, Statement stmt, ResultSet rs) {
		...
		if (con != null) {
			try {
				...
				logger.error("MySql : releaseConnection() : connection : ", e);
			}
		}
		if (stmt != null) {
			try {
				...
				logger.error("MySql : releaseConnection() : statement : ", e);
			}
		}
		if (rs != null) {
			try {
				...
				logger.error("MySql : releaseConnection() : resultset : ", e);
			}
		}
	}
