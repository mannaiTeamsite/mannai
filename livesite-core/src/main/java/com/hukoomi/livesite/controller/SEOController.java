...
public class SEOController {
	...
	/*
	 * Set Base SEO meta tags. For example, Locale, Canonical URL, Alternate links,
	 * etc. This is used as Site wide pre-controller.
	 *
	...
	public ForwardAction setBaseMetadata(RequestContext context) {
		...
		if (!currentPageLink.contains("-details.page")) {
			...
			context.getPageScopeData().put("href-lang-en",
					urlPrefix + commonUtils.getPrettyURLForPage(currentPageLink, "en", ""));
			context.getPageScopeData().remove("href-lang-ar");
			context.getPageScopeData().put("href-lang-ar",
					urlPrefix + commonUtils.getPrettyURLForPage(currentPageLink, "ar", ""));
