...
public class SolrQueryBuilder {
	...
	private final Logger logger = Logger.getLogger(SolrQueryBuilder.class);
	/**
	 * Map HashMap that will define the param-value set in the solr query url.
	 */
	private final Map<String, String> parameterMap = new HashMap<>();
	/**
	 * baseUrl String that will contain (host + core) value in the solr query url.
	 */
	private String baseUrl;
	/**
	 * baseQuery String that will hold search string of crawl query url.
	 */
	private String crawlUrl;
	/**
	 * baseQuery String that will hold search string of solr query url.
	 */
	private String baseQuery;
	/**
	 * rows int that will define the number of results fetched by the solr query
	 * url.
	 */
	private int rows = 0;
	/**
	 * start int that will define the start index of results fetched by the solr
	 * query url.
	 */
	private int start = 0;
	/**
	 * mltFl int String will define the mlt_fl attribute to the solr query url.
	 */
	private String mltFl;
	/**
	 * Declare fields String that will define the field filter of results fetched by
	 * the solr query url.
	 */
	private String fields;
	/**
	 * Declare enableMLT boolean that will define whether to set mlt_fl of results
	...
	/**
	 * sort String that will define sort field of results fetched by the solr query
	 * url.
	 */
	private String sort;
	/**
	 * recent String that will define recently visited sections by user to influence
	 * solr query
	 */
	private String recent;
	/**
	 * Persona Boost Boost value to influence solr query by persona
	 */
	private String personaBoost;
	/**
	 * Persona Boost Boost value to influence solr query by category
	 */
	...
	/**
	 * fieldQuery String that will define field & its values to be considered for
	 * the solr query url.
	 */
	private String fieldQuery;
	/**
	 * requestHandler String that will define solr request handler in the solr query
	 * url.
	 */
	private String requestHandler;
	/**
	 * groupingField String that will define the group field to group the results
	 * fetching from solr query url.
	 */
	private String groupingField;
	/**
	 * facetField String that will define the facet field to group the results
	 * fetching from solr query url.
	 */
	private String facetField;
	/**
	 * Declare highlight field that will come as suggestion text fetched by the solr
	 * query url.
	 */
	private String highlightField;
	/**
	 * Declare highlighter on/off that will highlight the suggestion text fetched by
	 * the solr query url.
	 */
	private String highlighter;
	/**
	 * Declare hlHtmlTag HTML tag that will highlight the string in suggestion text
	 * fetched by the solr query url.
	 */
	private String hlHtmlTag;
	/**
	 * Declare personaCookieValue HTML tag that will boost the search as per the
	 * persona cookie value fetched by solr query url.
	 */
	private String personaCookieValue;

	/**
	 * This method will be called from Component External Java for solr query
	 * building.
	 * 
	...
	private void init(final RequestContext context) {
		...
		PropertiesFileReader propertyFileReader = new PropertiesFileReader(context, "solrconfig.properties");
		Properties properties = propertyFileReader.getPropertiesFile();
		CommonUtils commonUtils = new CommonUtils();
		String lang = commonUtils.sanitizeSolrQuery(context.getParameterString("lang", ""));
		...
		if (context.isRuntime()) {
			solrHost = context.getParameterString("solrHost", properties.getProperty("solrRuntimeHost"));
		} else {
			solrHost = context.getParameterString("solrHost", properties.getProperty("solrAuthHost"));
		}
		...
		this.requestHandler = context.getParameterString("requestHandler", properties.getProperty("requestHandler"));
		logger.debug("Solr Request Handler: " + requestHandler);
		try {
			if (!isHeader) {
				this.baseQuery = commonUtils.sanitizeSolrQuery(
						URLDecoder.decode(context.getParameterString(BASE_QUERY, DEFAULT_QUERY), UTF));
				context.setParameterString(BASE_QUERY, this.baseQuery);
				this.fieldQuery = commonUtils
						.sanitizeSolrQuery(URLDecoder.decode(context.getParameterString(FIELD_QUERY, ""), UTF));
			...
			logger.error("Unable to decode baseQuery=" + context.getParameterString(BASE_QUERY, DEFAULT_QUERY), e);
			logger.error("Unable to decode fieldQuery=" + context.getParameterString(FIELD_QUERY), e);
		}

		try {
			this.groupingField = commonUtils
					.sanitizeSolrQuery(URLDecoder.decode(context.getParameterString("groupingField", ""), UTF));
			logger.debug("Solr Grouping Field: " + groupingField);
		} catch (UnsupportedEncodingException e) {
			logger.error("Unable to decode groupingField=" + context.getParameterString("groupingField"), e);
		}

		try {
			this.facetField = commonUtils
					.sanitizeSolrQuery(URLDecoder.decode(context.getParameterString("facet", ""), UTF));
			logger.debug("Solr Facet Fields: " + facetField);
		} catch (UnsupportedEncodingException e) {
			logger.error("Unable to decode facet Field=" + context.getParameterString("facet"), e);
		}
		...
		if (this.rows <= 0) {
			logger.warn(
					"SOLR row parameter value provided less " + "than or equal to zero, defaulting to configuration");
		...
		if (this.start < 0) {
			logger.warn("SOLR start parameter value provided" + " less than zero, defaulting to zero");
			this.start = 0;
		}
		logger.debug("Solr Result Rows: " + this.start);

		this.sort = context.getParameterString("sort", properties.getProperty("sort"));
		logger.info("Solr Result Sort: " + this.sort);
		this.mltFl = commonUtils
				.sanitizeSolrQuery(context.getParameterString("mlt_fl", properties.getProperty("mlt_fl")));
		...
		this.personaBoost = commonUtils.sanitizeSolrQuery(
				context.getParameterString("persona_boost", properties.getProperty("persona_boost")));
		logger.info("Boost for Persona: " + this.personaBoost);
		this.categoryBoost = commonUtils.sanitizeSolrQuery(
				context.getParameterString("category_boost", properties.getProperty("category_boost")));
		logger.info("Boost for Category: " + this.categoryBoost);
		this.queryBoost = commonUtils
				.sanitizeSolrQuery(context.getParameterString("query_boost", properties.getProperty("query_boost")));
	 ...
	 * @param paramValue set field query value to be matched in solr query.
	 *
	 * @return this set solr query params.
	 */
	public SolrQueryBuilder addQueryParam(final String paramName, final String paramValue) {
		this.parameterMap.put(paramName, paramValue);
		return this;
	}

	/**
	 * Specify requestHandler to be used for SolrQuery through external java code.
	 * This method should be used only when logic requires multiple SOLR queries
	 * using different requestHandlers. "requestHandler" can passed from Commponent
	 * as RequestContext parameter. If not provided the configuration defaults to
	 * requestHandler property provided in SOLR configurations.
	 * 
	 * @param requestHandlerValue set requst handler to solr query.
	 *
	 * @return this set requestHandler to solr query url.
	 */
	public SolrQueryBuilder useRequestHandler(final String requestHandlerValue) {
		this.requestHandler = requestHandlerValue;
		return this;
	}

	/**
	 * Provide field query through Component external code this method would
	 * override FieldQuery parameter provided in request context and also default
	 * property configuration if provided.
	 * 
	...
	/**
	 * Generate solr query from various context params and solr config properties
	 * file to return a solr query url.
	 *
	 * @return sb append all solr query params to a string builder return it as a
	 *         final url string.
	 */
	public String build() {
		...
		sb.append("?q=" + (StringUtils.isNotBlank(this.baseQuery) ? this.baseQuery : DEFAULT_QUERY));
		// sornaQube
		sb = solrQuery(sb);
		// sornarQube
		...
		if (StringUtils.isNotBlank(this.groupingField)) {
			sb.append("&group=true").append("&group.field=" + this.groupingField)
					.append("&group.limit=" + Integer.toString(this.rows));
		}
