...
public class WeatherApi {
	...
	public Document getWeather(final RequestContext context) {
		...
		String weatherUrl = properties.getProperty("weatherUrl",
				"https://qmet.com.qa/metservices/api/QMD/GetMeteoFactoryForecast");
		...
		return callWeatherApi(weatherUrl, "WeatherResponse", currentCity, temperatureScale, currentTempURL,
				defaultCity);
	}
	 ...
	 * @return Document
	 */
	public Document callWeatherApi(String weatherUrl, String xmlRootName, String currentCity, String temperatureScale,
			String currentTempURL, String defaultCity) {
		...
		try {
			...
			dataObject = returnWeather(dataObject, jsonArrayWeather, currentCity, tempMin, tempMax);
			...
			if (temperature != null) {
				...
				if (temp.isEmpty() || temp.equalsIgnoreCase("undefined")) {
					temp = document
							.valueOf("/data/area[@name='" + defaultCity + "']/parameter[@id='Temperature']/value");
	...
	public JSONObject returnWeather(JSONObject dataObject, JSONArray jsonArrayWeather, String currentCity,
		...
		for (int i = 0; i < jsonArrayWeather.length(); i++) {
			...
			if (city.equalsIgnoreCase(currentCity)) {
				...
				dataObject = returnData(dataObject, elem, weatherData);
...
public JSONObject returnData(JSONObject dataObject, String elem, JSONArray weatherData) {
