...
public class CustomerServiceExternal {
	...
	private Document getFieldValidationFailedDocument() {
		...
		Element resultElement = result.addElement(ELEMENT_RESULT);
		Element element = resultElement.addElement(ELEMENT_STATUS);
	...
	private boolean validateAction(String action) {
		...
		ESAPI.validator().getValidInput("CUSTOMER_SERVICE_ACTION", action, ESAPIValidator.ALPHABET, 20, false, true,
				errorList);
	...
	private boolean validateEService(String eServiceID) {
		...
		ESAPI.validator().getValidInput("E_SERVICE", eServiceID, ESAPIValidator.NUMERIC, 8, false, true, errorList);
		return errorList.isEmpty();
	}
	...
	private Document getServices(String eService, RequestContext context) {
		...
		builder.append("SELECT S.ESERVICEID as ESERVICEID,S.SERVICEID as SERVICEID,S.SERVICEEDESC as SERVICEEDESC,"
		...
		try {
			...
			if (null != connection) {
				...
				while (resultSet.next()) {
					...
					serviceADecs.setText(util.decodeToArabicString(resultSet.getString("SERVICEADESC")));
				}
			}
	...
	private Document getEServices(RequestContext context) {
		...
		builder.append("WHERE E.ESERVICEID=IEM.ESERVICEID AND E.ESERVICEADESC!='' AND E.ESERVICEEDESC!='' AND ");
		...
		try {
			...
			if (null != connection) {
				...
				while (resultSet.next()) {
					...
					eserviceADecs.setText(util.decodeToArabicString(resultSet.getString("eserviceadesc")));
	...
	private static Properties loadProperties(final RequestContext context) {
		...
		PropertiesFileReader propertyFileReader = new PropertiesFileReader(context, "customerserviceconfig.properties");
