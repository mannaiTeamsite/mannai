...
public class NewsletterPhpExternal {
    ...
    public Document subscribeToNewsletter(final RequestContext context) {
        ...
        logger.info("User Details From Front End :-> email : " + email + " subcription language : "
                + subscriptionLang + " persona : " + persona);
            ...
            if (bool) {
                ...
                if(subStatus!=null && !subStatus.equals("")) {
                    if (subStatus.equals(STATUS_SUBSCRIBED)) {
                        ...
                        sendConfirmationMail(email, pageLang, confirmationToken,
        ...
        if (validateLanguage(subscriptionLang) && validateMailID(xssUtils.stripXSS(email))) {
            ...
            if (verify) {
                if (uid != null && !uid.equals("") && getSubscriptionStatusByUid(uid)) {
                    ...
                    boolean subscriberMasterDataInsert =
                            addSubscriberInMasterTable(uid, subscriberId, email, STATUS_PENDING);
                    boolean subscriberPreferenceDataInsert = addSubscriberPreferences(subscriberId,
                    ...
                    if (STATUS_CONFIRMED.equals(confirmationStatus)) {
                        ...
                        if (STATUS_PENDING.equals(subscriptionStatus)) {
                            ...
                            String preferenceStatus =
                                    checkPrefernceStatus(email, persona, subscriptionLang);
                            if (STATUS_ACTIVE.equals(preferenceStatus)) {
                                ...
                                boolean preferenceUpdateStatus = updateSubscriberPreference(
                                        subscriberId, subscriptionLang, persona, STATUS_PENDING);
                                preferenceId =
                                        getPreferenceId(subscriberId, subscriptionLang, persona);
                                if (preferenceUpdateStatus) {
                                    String confirmationToken = generateConfirmationToken(
                                            subscriberId, preferenceId, email);
                                    sendConfirmationMail(email, pageLang, confirmationToken,
                                            CONFIRMATION_EMAIL, "", context);
     ...
     * @return
     */
    private boolean updateSubscriberPreference(
            double subscriberId, String subscriptionLang, String persona, String status
    ) {
        ...
        String addSubscriberPreferencesQuery =
                "INSERT INTO NEWSLETTER_PREFERENCE (SUBSCRIBER_ID, LANGUAGE, PERSONA, STATUS) VALUES(?,?,?,?)";
        Connection connection = null;
        PreparedStatement prepareStatement = null;
    ...
    private String checkPrefernceStatus(String email, String persona, String subscriptionLang) {
        ...
        String preferenceStatus = "";
        String tokenCheckQuery =
                "SELECT NP.STATUS FROM NEWSLETTER_MASTER NM INNER JOIN NEWSLETTER_PREFERENCE NP ON NM.SUBSCRIBER_ID = "
    ...
    private String checkConfirmationStatus(String email) {
        ...
        String checkConfirmationStatusQuery =
                "SELECT COUNT(*) FROM NEWSLETTER_CONFIRMATION_TOKEN WHERE "
    ...
    private double getPreferenceId(double subscriberId, String subscriptionLang, String persona) {
        ...
        double preferenceId = 0;
        String getPreferenceIdQuery =
                "SELECT PREFERENCE_ID FROM NEWSLETTER_PREFERENCE WHERE "
    ...
    public void sendConfirmationMail(
            String email, String pageLanguage, String confirmationToken, String emailElement,
        ...
        try {
            ...
            if (!unsubReason.equals("")) {
                message = message.replace("<token>", confirmationToken)
                        .replace("<lang>", pageLanguage).replace("<reason>", unsubReason);

            } else {
                message = message.replace("<token>", confirmationToken).replace("<lang>",
                        pageLanguage);
            }
     ...
     * @param persona
     */
    private boolean addSubscriberPreferences(
            double subscriberId, String subscriptionLanguage, String persona
    ) {
    ...
    private double generateSubscriberId() {
        ...
        String checkSubscriberIdQuery =
                "SELECT SUBSCRIBER_ID FROM NEWSLETTER_MASTER WHERE SUBSCRIBER_ID = ?";
     ...
     * @throws IOException              this method will verify the flag, based
     *                                  on flag it makes call to mailchimp
     */
    public boolean addSubscriberInMasterTable(
            String uid, double subscriberId, String email, String status
    ) {
        logger.info("NewsletterPhp External : addSubscriberInMasterTable");
        boolean subscriberMasterDataInsert = false;
        String addMasterDataQuery =
                "INSERT INTO NEWSLETTER_MASTER (SUBSCRIBER_ID, SUBSCRIBER_EMAIL, STATUS, SUBSCRIBED_DATE,UID) "
    ...
    private boolean validateMailID(String emailId) {
        ...
        ESAPI.validator().getValidInput(ELEMENT_EMAIL, emailId, ESAPIValidator.EMAIL_ID, 50, false,
                true, errorList);
    ...
    private boolean validateLanguage(String language) {
        ...
        ESAPI.validator().getValidInput("language", language, ESAPIValidator.ALPHABET, 2, false,
                true, errorList);
