...
public class ErrorExternal {
	...
	public Document errorData(final RequestContext context) {
		...
		try {
			...
			if (compType.equalsIgnoreCase("Banner") && context.isRuntime() && !contentPage.isBlank()
					&& !brokenLink.equalsIgnoreCase(errorPageEn) && !brokenLink.equalsIgnoreCase(errorPageAr)) {
