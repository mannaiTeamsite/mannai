...
/**
 * DashboardSettingsExternal is the components external class for dashboard
 * settings.
 * 
...
public class DashboardSettingsExternal {
    ...
    private static final Logger logger = Logger.getLogger(DashboardSettingsExternal.class);
    /**
    ...
    /**
     * This method will be called from Component External for fetching and
     * updating the persona settings.
     * 
     * @param context Request context object.
     *
     * @return doc Returns the solr response document generated from solr
     *         query.
     * 
     * @deprecated
     */
    ...
    public Document performDashboardSettingsAction(final RequestContext context) {
        ...
        logger.info("DashboardSettingsExternal : langSwitch " + langSwitch + " language " + language
                + " pageLang " + pageLang);
        ...
        if (valid.equalsIgnoreCase("valid")) {
            ...
            if (validateInput(context, settingsBO)) {
                ...
                if (ACTION_GET_ALL_SETTINGS.equalsIgnoreCase(settingsBO.getAction())) {
                ...
                } else if (ACTION_UPDATE_TOPICS.equalsIgnoreCase(settingsBO.getAction())) {
                ...
                } else if (ACTION_UNSUBSCRIBE.equalsIgnoreCase(settingsBO.getAction())) {
                    ...
                    if (status.equals(STATUS_SUCCESS)) {
                        ...
                        createUnsubscribeResponseDoc(responseElem, null, null, "", STATUS_SUCCESS,
                                "", subemail);
                    } else {
                        createUnsubscribeResponseDoc(responseElem, null, null, "", STATUS_FAILURE,
                                "", subemail);
                    }
                ...
                createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getAction(),
                        settingsBO.getPersona(), STATUS_FAILED, "Invalid input parameter");
            }
        } else {
            ...
            try {
                ...
                logger.error(
                        "Exception in performDashboardSettingsAction - Redirecting to Login Page",
                        e);
            }
        }
        logger.debug("Final Result :" + doc.asXML());
     ...
     * @param topics
     */
    public void processTopicsOperations(
            Element responseElem, DashboardSettingsBO settingsBO, String topics
    ) {

        boolean postgreDataInsertStatus = insertTopicsData(responseElem, settingsBO, topics);
    ...
    private String switchDashboardLanguage(String userId, String langSwitch, String language) {
        ...
        try {
            ...
            prepareStatement = connection.prepareStatement(updatePreferenceQuery);
            ...
            if (count > 0) {
                ...
                status = updatePhpLanguageList(getList(userId, lang, SWITCH), subscriberID,
                        langSwitch);
            }
            if (status.equals(STATUS_SUCCESS)) {
            ...
            logger.error("Exception in switchDashboardLanguage", e);
     ...
     * @return
     */
    private String updateMasterTableSwitch(String userId, String langSwitch, String language) {
        logger.info("DashboardSettingsExternal : updateMasterTableSwitch()");
        ...
        if (language.equals("en")) {
            updatePreferenceQuery = "UPDATE NEWSLETTER_MASTER SET ENGLISH_SWITCH = ? WHERE UID = ?";
        } else if (language.equals("ar")) {
            updatePreferenceQuery = "UPDATE NEWSLETTER_MASTER SET ARABIC_SWITCH = ? WHERE UID = ?";
        }
        ...
        try {
            ...
            logger.error("Exception in updateMasterTableSwitch", e);
            status = STATUS_FAILURE;
        } finally {
            postgre.releaseConnection(connection, prepareStatement, rs);
        }

        logger.debug("updateMasterTableSwitch() : update master table status " + status);
     ...
     * @param listName
     * @param subscriberID
     * @param langSwitch   This method is used to update the user list based on
     *                     language on/off
     * @return
     */
    private String
            updatePhpLanguageList(List<String> listName, double subscriberID, String langSwitch) {
        ...
        if (langSwitch.equals(LANGUAGE_ON)) {
            phpQuery =
                    "INSERT INTO PHPLIST_LISTUSER (USERID, LISTID, ENTERED) VALUES ((SELECT ID FROM PHPLIST_USER_USER "
                    + "WHERE EMAIL = ?),(SELECT ID FROM PHPLIST_LIST WHERE NAME = ?),LOCALTIMESTAMP)";

        } else if (langSwitch.equals(LANGUAGE_OFF)) {

            phpQuery =
                    "DELETE FROM PHPLIST_LISTUSER WHERE USERID = (SELECT ID FROM PHPLIST_USER_USER WHERE EMAIL = ?) "
                    + "and LISTID = (SELECT ID FROM PHPLIST_LIST WHERE NAME = ?)";
        }
        ...
        try {
            ...
            while (iteratorName.hasNext()) {
                ...
                logger.info("Inside updatePhpLanguageList() loop" + listname);
            ...
            logger.error("Exception in updatePhpLanguageList", e);
            status = STATUS_FAILURE;
        } finally {
            postgre.releaseConnection(connection, prepareStatement, rs);
        }
        logger.info("End of updatePhpLanguageList()" + status);
        return status;
    }
    ...
    private boolean insertPhpTopicsData(DashboardSettingsBO settingsBO) {
        ...
        try {
            ...
            if (delbool) {
                while (iteratorName.hasNext()) {
                    listname = iteratorName.next();
                    logger.info("DashboardSettingsExternal : inside loop" + listname);
                    prepareStatement.setString(1, subscriberemail);
                    prepareStatement.setString(2, listname);
                    count = prepareStatement.executeUpdate();
    ...
    private boolean
            deletePhpTopicsData(String query, String subscriberemail, List<Integer> topicList) {
        ...
        try {
            ...
            while (iteratorID.hasNext()) {
                ...
                logger.info("DashboardSettingsExternal : inside loop" + topicID);
                prepareStatement.setString(1, subscriberemail);
     ...
     * @param userId
     * @param language
     * @param module   This method is used to get the Topics selected by User
     *                 from Postgre
     * @return
     */
    private List<String> getList(String userId, String language, String module) {
        ...
        if (module.equals(TOPICS)) {
            getListQuery = "SELECT TOPIC_INTEREST_NAME FROM NEWSLETTER_INTEREST WHERE UID = ? ";
    ...
    /**
     * @param userId This method is used to get the Topics selected by User
     *               from SQL
     * @return
     */
    private List<Integer> getPhpList(String userId) {
        ...
        getListQuery =
                "SELECT ID FROM PHPLIST_LIST WHERE ID IN (SELECT LISTID FROM PHPLIST_LISTUSER WHERE USERID = "
                + "(SELECT ID FROM PHPLIST_USER_USER WHERE EMAIL = ?) AND CATEGORY= ?)";
    ...
    private String unsubscribeDashboardUser(String userId, String unsubReason) {
        logger.info("DashboardSettingsExternal : unsubscribeDashboardUser()" + userId);
        ...
        status = unsubscribePostgreUser(updateMasterQuery, userId, subscriberID, unsubReason,
                STATUS_UNSUBSCRIBED, NEWSLETTER_MASTER);
        logger.debug("unsubscribeDashboardUser() : update master status " + status);
        if (status.equals(STATUS_SUCCESS)) {
            logger.debug("Inside update preference status ");
            status = unsubscribePostgreUser(updatePreferenceQuery, userId, subscriberID,
                    unsubReason, STATUS_INACTIVE, NEWSLETTER_PREFERENCE);
        }
        logger.debug("unsubscribeDashboardUser() : update preference status " + status);

        if (status.equals(STATUS_SUCCESS)) {

            unsubStatus = unsubscribePhpUser(updatePhpQuery, subscriberemail, unsubReason);
        }
        ...
        if (unsubStatus.equals(STATUS_SUCCESS)) {
            ...
            syncStatus =
                    syncUnsubscribeData(syncPhpQuery, subscriberemail, unsubReason, "USERDATA");
            logger.debug("syncStatus 1 " + syncStatus);
            if (syncStatus.equals(STATUS_SUCCESS)) {

                syncStatus = syncUnsubscribeData(syncPhpBlacklist, subscriberemail, unsubReason,
                        "USERBLACKLIST");
                logger.debug("syncStatus 2 " + syncStatus);
            }
            if (syncStatus.equals(STATUS_SUCCESS)) {

                syncStatus = syncUnsubscribeData(syncPhpBlacklistData, subscriberemail, unsubReason,
                        "BLACKLISTDATA");
    ...
    private String unsubscribePostgreUser(
        ...
        logger.info("DashboardSettingsExternal : unsubscribeMasterUser()" + tabName
                + " subscriberId " + subscriberId + " status " + status);
        logger.info("DashboardSettingsExternal : unsubscribeMasterUser()" + unsubReason);
        ...
        logger.debug("DashboardSettingsExternal : unsubscribeMasterUser updated successfully"
                + tabName + " rsstatus " + rsstatus);
        return rsstatus;
    }
     ...
     */
    public boolean
            insertTopicsData(Element responseElem, DashboardSettingsBO settingsBO, String topics) {
        ...
        try {
            personaSettingsQuery =
                    "INSERT INTO NEWSLETTER_INTEREST (SUBSCRIBER_ID, UID, TOPIC_INTEREST_NAME, STATUS) "
                            + "VALUES (?, ?, ?, ?)";
            prepareStatement = connection.prepareStatement(personaSettingsQuery);

            for (int index = 0; index < topicsArray.length; index++) {
                prepareStatement.setDouble(1,
                        Double.parseDouble(subscriptionDetails.get("subscriberId")));
                prepareStatement.setString(2, subscriptionDetails.get(USERID));
            ...
            if (result.length == 0) {
                ...
                createTopicsResponseDoc(responseElem, null, null, personaValue, STATUS_FAILED,
                        "Newsletter interest topics insertion failed");
            } else {
                logger.info("Newsletter interest topics inserted successfully!");
                createTopicsResponseDoc(responseElem, null, null, personaValue, STATUS_SUCCESS, "");
                postgreDataInsertStatus = true;
            }
        } catch (Exception e) {
            logger.error("Exception in insertTopicsData", e);
            createTopicsResponseDoc(responseElem, null, null, null, STATUS_FAILED,
    ...
    private boolean deleteTopicsData(DashboardSettingsBO settingsBO) {
        ...
        try {
            ...
            if (result == 0) {
                ...
                logger.info("Newsletter interest topics deleted successfully!");
                deleteStatus = true;
            }
        } catch (Exception e) {
     ...
     * @param settingsBO
     */
    public void getTopicSettings(Element responseElem, DashboardSettingsBO settingsBO) {
        logger.info("DashboardSettingsExternal : getTopicSettings()");
        getSubscriptionDetails(settingsBO.getUserId());
        String subscriptionStatus = getSubscriptionStatus(settingsBO.getUserId(), TOPICS);
        String topics = "";

        if ("Subscribed".equals(subscriptionStatus)) {
            topics = getTopicsInterest(settingsBO.getUserId());

            createTopicsResponseDoc(responseElem, topics, settingsBO.getUserId(),
                    settingsBO.getPersona(), STATUS_SUBSCRIBED, "");

            String languageSwitchDetails = getLangSwitchDetails(settingsBO.getUserId());

            createLanguageSwitchDetailsDoc(responseElem, languageSwitchDetails);

        } else if (STATUS_PENDING.equals(subscriptionStatus)) {
            createTopicsResponseDoc(responseElem, topics, settingsBO.getUserId(),
                    settingsBO.getPersona(), CONFIRMATION_PENDING, "");
        } else {
            createTopicsResponseDoc(responseElem, topics, settingsBO.getUserId(),
                    settingsBO.getPersona(), STATUS_NOT_SUBSCRIBED, "");
        }
     ...
     */
    private void
            createLanguageSwitchDetailsDoc(Element responseElem, String languageSwitchDetails) {
    ...
    private String getLangSwitchDetails(String userId) {
        ...
        String switchDetailsQuery =
                "SELECT ENGLISH_SWITCH, ARABIC_SWITCH FROM NEWSLETTER_MASTER WHERE UID = ?";
        logger.info("switchDetailsQuery : " + switchDetailsQuery);
        ...
        try {
            connection = postgre.getConnection();
            prepareStatement = connection.prepareStatement(switchDetailsQuery);
            prepareStatement.setString(1, userId);
            rs = prepareStatement.executeQuery();
    ...
    private Map<String, String> getSubscriptionDetails(String userId) {
        ...
        String subscriptionDetailsQuery =
                "SELECT SUBSCRIBER_ID, SUBSCRIBER_EMAIL, STATUS, SUBSCRIBED_DATE, UID FROM NEWSLETTER_MASTER "
                + "WHERE UID = ?";
        logger.info("subscriptionDetailsQuery : " + subscriptionDetailsQuery);
        ...
        try {
            connection = postgre.getConnection();
            prepareStatement = connection.prepareStatement(subscriptionDetailsQuery);
            prepareStatement.setString(1, userId);
            rs = prepareStatement.executeQuery();

            while (rs.next()) {
                subscriptionDetails.put("subscriberId", rs.getString(1));
                subscriptionDetails.put("subscriberEmail", rs.getString(2));
     ...
     * @author Arbaj
     * 
     *         This method is used to create the topics response document based
     *         on the input.
     * 
     * @param responseElem
     * @param action
     ...
     * @param error
     */
    private void createTopicsResponseDoc(
            Element responseElem, String topics, String userId, String persona, String status,
            String error
    ) {
        if (topics == null)
            topics = "";
     ...
     * @author Pramesh
     * 
     *         This method is used to send unsubscription response document
     *         based on the input.
     * 
     * @param responseElem
     * @param action
     ...
     * @param error
     */
    private void createUnsubscribeResponseDoc(
            Element responseElem, String topics, String userId, String persona, String status,
    ...
    public boolean validateInput(final RequestContext context, DashboardSettingsBO settingsBO) {
        ...
        validData = ESAPI.validator().getValidInput(SETTINGS_ACTION, settingsAction,
        ...
        if (request.getSession().getAttribute(USERID) != null
                && !"".equals(request.getSession().getAttribute(USERID))) {
        ...
        if (request.getSession().getAttribute(USER_TYPE) != null) {
            String userType = request.getSession().getAttribute(USER_TYPE).toString();
            logger.debug(USER_TYPE + " >>>" + userType + "<<<");
            validData = ESAPI.validator().getValidInput(USER_TYPE, userType,
                    ESAPIValidator.ALPHABET, 10, true, true, errorList);
            if (errorList.isEmpty()) {
        ...
        if (ACTION_UPDATE_PERSONA.equalsIgnoreCase(settingsAction)) {
            ...
            validData = ESAPI.validator().getValidInput(PERSONA, persona,
    ...
    public void getAllSettings(Element responseElem, DashboardSettingsBO settingsBO) {
        ...
        try {
            ...
            if (personaValue != null && !"".equals(personaValue)) {
                createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getUserId(),
                        personaValue, STATUS_SUCCESS, "");
            } else {
                createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getUserId(),
                        personaValue, STATUS_FAILED, "Persona value not available");
            }
        } catch (Exception e) {
    ...
    /**
     * This method is used to retrieve person setting for the user from
     * database.
     *
    ...
    public void updatePersonaForUser(Element responseElem, DashboardSettingsBO settingsBO) {
        ...
        if (isPersonaSettingExists(settingsBO.getUserId())) {
            try {
                personaSettingsQuery =
                        "UPDATE persona_settings SET persona_value = ?, active = ?, modified_on = current_timestamp "
                                + "WHERE user_id = ?";
                prepareStatement = connection.prepareStatement(personaSettingsQuery);
                ...
                if (result == 0) {
                    ...
                    createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getUserId(),
                            personaValue, STATUS_FAILED, "Persona settings updation failed");
                } else {
                ...
                createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getUserId(),
                        null, STATUS_FAILED, "Exception in updating persona settings");
            } finally {
                postgre.releaseConnection(connection, prepareStatement, null);
            }
        } else {
            try {
                personaSettingsQuery =
                        "INSERT INTO persona_settings(user_id, persona_value, active, created_on, modified_on) "
                ...
                if (result == 0) {
                    ...
                    createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getUserId(),
                            personaValue, STATUS_FAILED, "Persona settings insertion failed");
                } else {
                ...
                createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getUserId(),
                        null, STATUS_FAILED, "Exception in insert persona settings");
            } finally {
                postgre.releaseConnection(connection, prepareStatement, null);
            }
        }
    ...
    public void createResponseDoc(
            Element responseElem, String action, String userId, String persona, String status,
            String error
    ) {
        if (action == null)
            action = "";
        ...
        personaresponseElem.addElement(USER_ID).setText(userId);
        personaresponseElem.addElement(PERSONA).setText(persona);
        personaresponseElem.addElement(ERROR).setText(error);
    }

    /**
     * This method is used to check if a persona settins is available for the
     * user in database.
     * 
     * 
     * @return Returns true if the persona settings exists for the user else
     *         returns false.
     * 
     */
     ...
     */
    public String
            generateConfirmationToken(double subscriberId, double preferenceId, String email) {
        ...
        if (tokenExist) {
            ...
            String addGeneratedTokenQuery =
                    "INSERT INTO NEWSLETTER_CONFIRMATION_TOKEN (TOKEN, SUBSCRIBER_ID, PREFERENCE_ID, GENERATED_DATE, "
    ...
    public boolean isEmailAlreadyExist(String email) {
        ...
        String emailCheckQuery =
                "SELECT SUBSCRIBER_EMAIL FROM NEWSLETTER_MASTER WHERE SUBSCRIBER_EMAIL = ?";
        Connection connection = null;
        PreparedStatement prepareStatement = null;
        ResultSet resultSet = null;
        try {
