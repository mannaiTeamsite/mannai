...
public class DashboardSettingsExternal {
	...
	/**
	 * This method will be called from Component External for fetching and updating
	...
	public void loginSettings(final RequestContext context, Element responseElem, String langSwitch, String language) {
		...
		if (validateInput(context, settingsBO)) {
			...
			if (ACTION_GET_ALL_SETTINGS.equalsIgnoreCase(settingsBO.getAction())) {
				...
				unsubscribeDoc(status, responseElem, "");
			}

		} else {
			logger.info("Invalid input parameter");
			createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getAction(), settingsBO.getPersona(),
	...
	public void unsubscribeDoc(String status, Element responseElem, String subemail) {
		if (status.equals(STATUS_SUCCESS)) {
			...
			createUnsubscribeResponseDoc(responseElem, null, null, "", STATUS_SUCCESS, "", subemail);
		} else {
	 ...
	 * @param topics
	 */
	public void processTopicsOperations(Element responseElem, DashboardSettingsBO settingsBO, String topics) {
	...
	private String switchDashboardLanguage(String userId, String langSwitch, String language) {
		...
		if (langSwitch.equals(LANGUAGE_ON)) {
			...
			updatePreferenceQuery = "UPDATE NEWSLETTER_PREFERENCE SET STATUS = ? WHERE SUBSCRIBER_ID = ? AND LANGUAGE = ?";
		...
		try {
			...
			if (count > 0) {
				...
				status = updatePhpLanguageList(getList(userId, lang, SWITCH), subscriberID, langSwitch);
			}
			if (status.equals(STATUS_SUCCESS)) {
	 ...
	 * @return
	 */
	private String updatePhpLanguageList(List<String> listName, double subscriberID, String langSwitch) {
		...
		if (langSwitch.equals(LANGUAGE_ON)) {
			phpQuery = "INSERT INTO PHPLIST_LISTUSER (USERID, LISTID, ENTERED) VALUES ((SELECT ID FROM PHPLIST_USER_USER "
					+ "WHERE EMAIL = ?),(SELECT ID FROM PHPLIST_LIST WHERE NAME = ?),LOCALTIMESTAMP)";

		} else if (langSwitch.equals(LANGUAGE_OFF)) {

			phpQuery = "DELETE FROM PHPLIST_LISTUSER WHERE USERID = (SELECT ID FROM PHPLIST_USER_USER WHERE EMAIL = ?) "
	...
	private boolean insertPhpTopicsData(DashboardSettingsBO settingsBO) {
		...
		phpTopicsInsertQuery = "INSERT INTO PHPLIST_LISTUSER (USERID, LISTID, ENTERED) VALUES ((SELECT ID FROM PHPLIST_USER_USER "
				+ "WHERE EMAIL = ?),(SELECT ID FROM PHPLIST_LIST WHERE NAME = ?),LOCALTIMESTAMP)";

		phpTopicsDeleteQuery = "DELETE FROM PHPLIST_LISTUSER WHERE USERID = (SELECT ID FROM PHPLIST_USER_USER WHERE EMAIL = ?) "
				+ "AND LISTID = ? ";
	 ...
	 * @param userId
	 * @param language
	 * @param module   This method is used to get the Topics selected by User from
	 *                 Postgre
	 * @return
	 */
	...
	/**
	 * @param userId This method is used to get the Topics selected by User from SQL
	 * @return
	 */
	private List<Integer> getPhpList(String userId) {
		...
		getListQuery = "SELECT ID FROM PHPLIST_LIST WHERE ID IN (SELECT LISTID FROM PHPLIST_LISTUSER WHERE USERID = "
	...
	private String unsubscribeDashboardUser(String userId, String unsubReason) {
		...
		String updateMasterQuery = "UPDATE NEWSLETTER_MASTER SET STATUS = ? , UNSUBSCRIBED_REASON = ? WHERE UID = ?";
		...
		status = unsubscribePostgreUser(updateMasterQuery, userId, subscriberID, unsubReason, STATUS_UNSUBSCRIBED,
				NEWSLETTER_MASTER);
		logger.debug("unsubscribeDashboardUser() : update master status " + status);
		if (status.equals(STATUS_SUCCESS)) {
			logger.debug("Inside update preference status ");
			status = unsubscribePostgreUser(updatePreferenceQuery, userId, subscriberID, unsubReason, STATUS_INACTIVE,
	 ...
	 */
	private String unsubscribePostgreUser(String queryVal, String userid, double subscriberId, String unsubReason,
			String status, String tabName) {
		logger.info("DashboardSettingsExternal : unsubscribeMasterUser()" + tabName + " subscriberId " + subscriberId
				+ " status " + status);
		...
		logger.debug("DashboardSettingsExternal : unsubscribeMasterUser updated successfully" + tabName + " rsstatus "
				+ rsstatus);
		return rsstatus;
	}
	...
	private String getSubscriberEmail(String uid) {
		...
		String addSubscriberPreferencesQuery = "SELECT SUBSCRIBER_EMAIL FROM NEWSLETTER_MASTER WHERE UID = ?";
		Connection connection = null;
		PreparedStatement prepareStatement = null;
		String subscriberEmail = "";
		ResultSet rs = null;
	 ...
	 * @param topics
	 */
	public boolean insertTopicsData(Element responseElem, DashboardSettingsBO settingsBO, String topics) {
		...
		try {
			personaSettingsQuery = "INSERT INTO NEWSLETTER_INTEREST (SUBSCRIBER_ID, UID, TOPIC_INTEREST_NAME, STATUS) "
					+ "VALUES (?, ?, ?, ?)";
			prepareStatement = connection.prepareStatement(personaSettingsQuery);

			for (int index = 0; index < topicsArray.length; index++) {
				prepareStatement.setDouble(1, Double.parseDouble(subscriptionDetails.get("subscriberId")));
			...
			createTopicsResponseDoc(responseElem, null, null, null, STATUS_FAILED, "Exception in insertTopicsData");
		} finally {
	...
	public void getTopicSettings(Element responseElem, DashboardSettingsBO settingsBO) {
		...
		if ("Subscribed".equals(subscriptionStatus)) {
			...
			createTopicsResponseDoc(responseElem, topics, settingsBO.getUserId(), settingsBO.getPersona(),
					STATUS_SUBSCRIBED, "");
	 ...
	 */
	private void createLanguageSwitchDetailsDoc(Element responseElem, String languageSwitchDetails) {
	...
	private Map<String, String> getSubscriptionDetails(String userId) {
		...
		String subscriptionDetailsQuery = "SELECT SUBSCRIBER_ID, SUBSCRIBER_EMAIL, STATUS, SUBSCRIBED_DATE, UID FROM NEWSLETTER_MASTER "
				+ "WHERE UID = ?";
	 ...
	 * @param error
	 */
	private void createUnsubscribeResponseDoc(Element responseElem, String topics, String userId, String persona,
			String status, String error, String email) {
		if (topics == null)
			topics = "";
		if (userId == null)
			userId = "";
	...
	public boolean validateInput(final RequestContext context, DashboardSettingsBO settingsBO) {
		...
		validData = ESAPI.validator().getValidInput(SETTINGS_ACTION, settingsAction, ESAPIValidator.ALPHABET, 20, false,
				true, errorList);
		...
		if (request.getSession().getAttribute(USER_TYPE) != null) {
			...
			validData = ESAPI.validator().getValidInput(USER_TYPE, userType, ESAPIValidator.ALPHABET, 10, true, true,
					errorList);
			if (errorList.isEmpty()) {
		...
		if (ACTION_UPDATE_PERSONA.equalsIgnoreCase(settingsAction)) {
			...
			validData = ESAPI.validator().getValidInput(PERSONA, persona, ESAPIValidator.ALPHABET_HYPEN, 50, false,
					true, errorList);
			if (errorList.isEmpty()) {
	...
	public void getAllSettings(Element responseElem, DashboardSettingsBO settingsBO) {
		...
		try {
			...
			if (personaValue != null && !"".equals(personaValue)) {
				createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getUserId(), personaValue,
						STATUS_SUCCESS, "");
			} else {
				createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getUserId(), personaValue,
						STATUS_FAILED, "Persona value not available");
			}
		} catch (Exception e) {
	...
	public void updatePersonaForUser(Element responseElem, DashboardSettingsBO settingsBO) {
		...
		if (isPersonaSettingExists(settingsBO.getUserId())) {
			try {
				personaSettingsQuery = "UPDATE persona_settings SET persona_value = ?, active = ?, modified_on = current_timestamp "
						+ "WHERE user_id = ?";
				prepareStatement = connection.prepareStatement(personaSettingsQuery);
				...
				if (result == 0) {
					...
					createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getUserId(), personaValue,
							STATUS_FAILED, "Persona settings updation failed");
				} else {
				...
				createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getUserId(), null, STATUS_FAILED,
						"Exception in updating persona settings");
			} finally {
				postgre.releaseConnection(connection, prepareStatement, null);
			}
		} else {
			try {
				personaSettingsQuery = "INSERT INTO persona_settings(user_id, persona_value, active, created_on, modified_on) "
				...
				if (result == 0) {
					...
					createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getUserId(), personaValue,
							STATUS_FAILED, "Persona settings insertion failed");
				} else {
				...
				createResponseDoc(responseElem, settingsBO.getAction(), settingsBO.getUserId(), null, STATUS_FAILED,
						"Exception in insert persona settings");
			} finally {
				postgre.releaseConnection(connection, prepareStatement, null);
			}
		}
	 ...
	 * 
	 */
	public void createResponseDoc(Element responseElem, String action, String userId, String persona, String status,
			String error) {
		if (action == null)
			action = "";
		if (userId == null)
	...
	/**
	 * This method is used to check if a persona settins is available for the user
	 * in database.
	 * 
	 * 
	 * @return Returns true if the persona settings exists for the user else returns
	 *         false.
	 * 
	 */
