...
public class HukoomiExternal {
	...
	public Document getLandingContent(RequestContext context) {
		...
		if (root != null && root.element(CORRECTWORD_CONSTANT) != null
				&& StringUtils.isNotBlank(root.element(CORRECTWORD_CONSTANT).getText())) {
	...
	@SuppressWarnings("deprecation")
	private SolrQueryBuilder landingContentQuery(RequestContext context, SolrQueryBuilder sqb, String fieldQuery,
			String category) {
		...
		String crawlFields = commonUtils
				.sanitizeSolrQuery(context.getParameterString("crawlFields", "title,url,description"));
