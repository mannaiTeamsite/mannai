...
public class DetailExternalRelatedContent {
	...
	public Document getContentDetail(final RequestContext context) {
		...
		if (relDCRContent != null && detailDocument.selectSingleNode(relDCRContent) != null) {
			...
			detailDocument.getRootElement().add(detailDoc.getRootElement());
		}
	...
	/**
	 * This method will be called from Component External for related solr Content
	 * fetching.
	 * 
	 * @param context        The parameter context object passed from Component.
	 ...
	 */
	@SuppressWarnings("deprecation")
	public Document getRelatedContent(final RequestContext context, final Document detailDocument,
		...
		String category = commonUtils.getValueFromXML("/content/root/category", detailDocument);
		if (StringUtils.isNotBlank(category)) {
			sb.append("&fq=category:" + category);
		}
		String relatedContent = context.getParameterString("relatedContent", "");
		if (StringUtils.isNotBlank(relatedContent)) {
			addContent(context, sb.toString(), relatedContent, "relatedContent", detailDocument, properties);
		}
		String trendContent = context.getParameterString("trendContent", "");
		if (StringUtils.isNotBlank(trendContent)) {
			addContent(context, sb.toString(), trendContent, "trendingContent", detailDocument, properties);
		}
		return detailDocument;
	}
	 ...
	 */
	@SuppressWarnings("deprecation")
	public void addContent(final RequestContext context, final String fielQuery, final String nodeList,
			final String root, final Document detailDocument, final Properties properties) {
		SolrQueryUtil squ = new SolrQueryUtil();
		CommonUtils commonUtils = new CommonUtils();
		String rows = context.getParameterString("rows", "9");
		String[] values = nodeList.split(",");
		StringBuilder fq = new StringBuilder(fielQuery);
		String dcrValue = "";
		String sortVal = getSortContent(context, detailDocument, properties);
		for (int i = 0; i < values.length; i++) {
			if (values[i].split(":")[2].equals("Node")) {
				dcrValue = commonUtils.getValueFromXML(values[i].split(":")[1], detailDocument);
			} else {
	...
	/**
	 * This method will be called to set the solr sort param content.
	 * 
	 * @param context        The parameter context object passed from Component.
	 * @param detailDocument The final content document to be returned.
	...
	private String getsortVal(final Document detailDocument,final Properties properties, String sort) {
		...
		if (StringUtils.isNotBlank(location)) {
			...
			sortVal = sortVal + location + properties.getProperty(locationSuffix);
