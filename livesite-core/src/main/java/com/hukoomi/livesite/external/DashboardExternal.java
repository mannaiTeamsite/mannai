...
public class DashboardExternal {
    ...
    public void
            dashboardServices(RequestContext context, String accessToken) {
        ...
        if (accessToken != null)
            try {
                ...
                setSessionAttributes(jwtParsedToken, request,
                        "Some other exception in JWT parsing");
            }
    }

    private void setSessionAttributes(
            String jwtParsedToken, HttpServletRequest request,
            String status
    ) {
        LOGGER.info(
                "--------------setSessionAttributes is called------------");
        ...
        if (status.equals("valid")) {
            ...
            session.setAttribute("mobile",
                    getValue(jwtParsedToken, "mobile"));
            session.setAttribute("email",
                    getValue(jwtParsedToken, "email"));
            session.setAttribute("lstMdfy",
                    getValue(jwtParsedToken, "lstMdfy"));
            session.setAttribute("role", getValue(jwtParsedToken, "role"));
            session.setAttribute("exp", getValue(jwtParsedToken, "exp"));
            session.setAttribute("usertypeNo",
                    getValue(jwtParsedToken, "type"));
            session.setAttribute("userId",
                    getValue(jwtParsedToken, "uid"));
    ...
    public Document doLogout(RequestContext context) throws IOException {
        ...
        String url =
                properties.getProperty("logout") + "?relayURL=" + relayURL;
    ...
    public Document getDashboardContent(RequestContext context) {
        ...
        if (status != null && status.equals("valid")) {
            ...
            Element userTypeNoElement =
                    userdata.addElement("userTypeNoElement");
            userTypeNoElement.setText("1");
        }

        LOGGER.info("Final doc" + doc.asXML());
        LOGGER.info(
                "--------------getDashboardConetent Ended------------");
        return doc;
    ...
    public Document getMyDataContent(RequestContext context) {
        ...
        if (status != null && status.equals("valid")) {
            ...
            String userTypeNo =
                    (String) session.getAttribute("userTypeNo");
            if (userTypeNo != null) {
    ...
    public Document getDashboardbookmark(RequestContext context) {
        ...
        Element bookmarkResultEle =
                bookmarkSearchDoc.addElement("bookmark");
    ...
    private void getBookmark(Element element, String category) {
        ...
        String searchQuery =
                "select page_title, page_url, page_description, active, content_type, category from"
                        + " " + table + " " + "where" + " " + "locale='"
                        + locale + "' and " + "user_id='" + userID
    ...
    private void removeBookmark(Element bookmarkResultEle) {
        ...
        Connection connection = getConnection();
        PreparedStatement prepareStatement = null;
        String updateQuery = "UPDATE" + " " + table + " set active='"
                + activeflag + "' where locale='" + locale
                + "' and user_id='" + userID + "' and page_title='"
                + pagetitle + "' and page_url='" + pageurl
    ...
    public boolean isBookmarkPresent() {
        ...
        Connection connection = getConnection();
        PreparedStatement prepareStatement = null;
        String searchQuery = "select active from" + " " + table + " "
                + "where" + " " + "locale='" + locale + "' and user_id='"
                + userID + "' and page_title='" + pagetitle
                + "' and page_url='" + pageurl + "' and content_type='"
                + contenttype + "'";
        ResultSet resultSet = null;
        try {
            if (connection != null) {
    ...
    public void nonLoggedInRedirect(RequestContext context)
            throws IOException {
        ...
        if (session.getAttribute("status") != "valid") {
            final String RELAY_URL = "relayURL";
            PropertiesFileReader prop = null;
            prop = new PropertiesFileReader(context,
                    "dashboard.properties");
            properties = prop.getPropertiesFile();
            RequestHeaderUtils rhu = new RequestHeaderUtils(context);
            String relayURL = rhu.getCookie(RELAY_URL);
            String url = properties.getProperty("login") + "?relayURL="
                    + relayURL;
