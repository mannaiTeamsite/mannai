...
public class PollSurveyTask implements CSURLExternalTask {
     ...
     * Option inserted
     */
    private static final String LOG_OPTION_INSTERTED = "Survey Option Inserted";
    /**
     * Question inserted
     */
    private static final String LOG_QUESTION_INSTERTED = "Survey Question Inserted";
    /**
     * SurveyBO
     */
    private static final String LOG_SURVEYBO = "SurveyBO : ";
    /**
     * Query where clause
     */
    private static final String QUERY_WHERE_CLAUSE = "WHERE SURVEY_ID = ? AND LANG = ?";
    /**
     * Button
     */
    private static final String LOG_BUTTON = "button";
    /**
     * Log Question
     */
    private static final String LOG_QUESTION = "question : ";
    /**
        ...
        for (CSAreaRelativePath taskFilePath : taskFileList) {
            try {
                ...
                if (file.getKind() == CSHole.KIND) {
                    statusMap = (HashMap<String, String>) processDeletedFiles(file, client);
                }else {
                    statusMap = (HashMap<String, String>) processFiles(file);
                }
    ...
    /**
     * This method is to process the deleted files.
     * 
     * @param file CSFile object.
     * @param client CSClient object.
     * 
     * @return Returns transaction statusMap.
     */
    public Map<String, String> processDeletedFiles(CSFile file, CSClient client){
        HashMap<String, String> statusMap = null;
        try {
            ...
            if(!comment.startsWith("[moved to")) {
                ...
                CSSimpleFile csSimpleTaskFile = getDeletedFile(taskHoleFile);
                String dcrType = null;
                if(csSimpleTaskFile != null) {
                    dcrType = csSimpleTaskFile
            ...
            logger.error("Exception in processDeletedFiles: ", e);
        }
        return statusMap;
    }
    ...
    /**
     * This method is to process the files.
     * 
     * @param file CSFile object.
     * @param client CSClient object.
     * 
     * @return Returns transaction statusMap.
     */
    public Map<String, String> processFiles(CSFile file){
        HashMap<String, String> statusMap = null;
        try {
            ...
            logger.error("Exception in processFiles: ", e);
        }
        return statusMap;
    }
    
    /**
     * This method is to create a list of options list.
     */
    ...
    /**
     * This method is to get the deleted file
     *
     * @param taskHoleFile Task hole file of CSHoleImpl object
     * @return Returns CSSimpleFile object.
     */
    private CSSimpleFile getDeletedFile(CSHoleImpl taskHoleFile) {
        ...
        try {
            ...
            if (taskHoleFile.getKind() == CSHole.KIND) {
                ...
                if(hole.getPreviousKind() == CSSimpleFile.KIND) {
                    ...
                    CSEdition edition = getEdition(hole, editions);
    ...
    /**
     * This method is to get the matcing edition of the file
     *
     * @param taskHoleFile Task hole file of CSHoleImpl object
     * @return Returns CSEdition object.
     * @throws CSException 
     ...
     */
    private CSEdition getEdition(CSHole hole, CSEdition[] editions) {
        ...
        try {
            ...
            logger.error("Exception in getEdition : ", e);
        }
        return edition;
    }
    ...
    public boolean insertSurveyData(Document document, String fileName) {
        ...
        try {
            ...
            logger.debug(LOG_SURVEYBO + surveyBO);
        ...
        try {
            ...
            for (Node node : nodes) {
                ...
                String questionType = node
                        .selectSingleNode(FIELD_TYPE).getText();
                logger.debug("insertSurveyQuestionData questionType : "
                        + questionType);

                if (StringUtils.isNotBlank(questionType) && !LOG_BUTTON.equalsIgnoreCase(questionType)) {
                    ...
                    String question = node.selectSingleNode(QUESTION)
                            .getText();
                    logger.debug(LOG_QUESTION + question);
                    ...
                    if (questionResult > 0) {
                        result = true;
                        logger.info(LOG_QUESTION_INSTERTED);
                        result = insertSurveyOptions(surveyBO, questionType, questionNo, questionId, node, connection);
                        if(!result) break;
                        questionNo++;
                    } else {
                        result = false;
    ...
    /**
     * Method inserts the survey options
     *
     * @param surveyBO   Survey business object
     ...
     * @throws SQLException
     */
    public boolean insertSurveyOptions(TSSurveyBO surveyBO, String questionType, int questionNo,
            Long questionId, Node node, Connection connection) throws SQLException {
        boolean result = false;
            ...
            for (Node node : nodes) {
                ...
                if (StringUtils.isNotBlank(questionType) && !LOG_BUTTON.equalsIgnoreCase(questionType)) {
                    ...
                    if (questionResult > 0) {
                        result = true;
                        logger.info(LOG_QUESTION_INSTERTED);
                        result = insertDynamicSurveyOptions(surveyBO, questionType, node, connection);
                        if(!result) break;
                    } else {
                        result = false;
                        connection.rollback();
    ...
    /**
     * Method inserts the dynamic survey options
     *
     * @param surveyBO   Survey business object
     * @param questionType  Question Type
     * @param node  Node object
     ...
     * @throws SQLException
     */
    public boolean insertDynamicSurveyOptions(TSSurveyBO surveyBO, String questionType, 
            Node node, Connection connection) throws SQLException {
        boolean result = false;
        if(optionsList.contains(questionType)) {
    ...
    public boolean insertSurveyOptionData(TSSurveyBO surveyBO, Node node,
        ...
        try {
            ...
            if (optionBatch.length == optionNo - 1) {
                logger.info(LOG_OPTION_INSTERTED);
                result = true;
            }
            ...
            for (Node node : nodes) {
                ...
                if (StringUtils.isNotBlank(questionType) && !LOG_BUTTON.equalsIgnoreCase(questionType)) {
                    ...
                    if (questionResult > 0) {
                        result = true;
                        logger.info(LOG_QUESTION_INSTERTED);
                        result = updateSurveyOptions(surveyBO, questionType, questionNo, node, connection);
                        if(!result) break;
                        questionNo++;
                    } else {
                        result = false;
    ...
    /**
     * Method updates the survey options
     *
     * @param surveyBO   Survey business object
     * @param questionType   Question Type
     * @param questionNo   Question Number
     ...
     * @throws SQLException
     */
    public boolean updateSurveyOptions(TSSurveyBO surveyBO, String questionType, int questionNo,
           Node node, Connection connection) throws SQLException {
        boolean result = false;
        if(optionsList.contains(questionType)) {
            surveyBO.setQuestionNo(questionNo);
            ...
            for (Node node : nodes) {
                ...
                if (StringUtils.isNotBlank(questionType) && !LOG_BUTTON.equalsIgnoreCase(questionType)) {
                    ...
                    if (questionResult > 0) {
                        result = true;
                        logger.info(LOG_QUESTION_INSTERTED);
                        result = updateDynamicSurveyOptions(surveyBO, questionType, node, connection);
                        if(!result) break;
    ...
    /**
     * Method updates the dynamic survey options
     *
     * @param surveyBO   Survey business object
     * @param questionType   Question Type
     * @param node   Node object
     ...
     * @throws SQLException
     */
    public boolean updateDynamicSurveyOptions(TSSurveyBO surveyBO, String questionType, 
           Node node, Connection connection) throws SQLException {
        boolean result = false;
        if(optionsList.contains(questionType)) {
     ...
     */
    public Map<String, String> updateLastModifierMap(String lastModifierStr) {
