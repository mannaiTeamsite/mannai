...
public class PollSurveyTask implements CSURLExternalTask {
	 ...
	 * Option String
	 */
	private static final String OPTION_STR = ".//option";
	/**
	 * Question String
	 */
	 ...
	 * Whereclause String
	 */
	private static final String WHERE_CLAUSE_SURVEY = "WHERE SURVEY_ID = ? AND LANG = ?";
	/**
	 * Button String
	 */
	...
	public void execute(CSClient client, CSExternalTask task, Hashtable params) throws CSException {
			...
			try {
				...
				if (file.getKind() == CSHole.KIND) {
					...
					if (!comment.startsWith("[moved to")) {
						...
						String dcrType = csSimpleTaskFile.getExtendedAttribute(META_DATA_NAME_DCR_TYPE).getValue();

						if (dcrType != null) {
							if (POLLS_CONTENT_TYPE.equalsIgnoreCase(dcrType)) {
								Document doc = getTaskDocument(csSimpleTaskFile);
								logger.info("POLLS_CONTENT_TYPE :doc of hole file : " + doc.asXML());
								String pollId = getDCRValue(doc, ID_PATH);
								String lang = getDCRValue(doc, LANG_PATH);
								statusMap = (HashMap<String, String>) updateDeletePollMasterData(pollId, lang,
										postgre.getConnection());
							} else if (SURVEY_CONTENT_TYPE.equalsIgnoreCase(dcrType)) {
								Document doc = getTaskDocument(csSimpleTaskFile);
								logger.info("SURVEY_CONTENT_TYPE doc of hole file : " + doc.asXML());
								String surveyId = getDCRValue(doc, ID_PATH);
								String lang = getDCRValue(doc, LANG_PATH);
								statusMap = (HashMap<String, String>) updateDeleteSurveyMasterData(surveyId, lang,
										postgre.getConnection());
							} else if (DYNAMIC_SURVEY_CONTENT_TYPE.equalsIgnoreCase(dcrType)) {
								Document doc = getTaskDocument(csSimpleTaskFile);
								logger.debug("DYNAMIC_SURVEY_CONTENT_TYPE doc of hole file : " + doc.asXML());
								String surveyId = getDCRValue(doc, ID_PATH);
								String lang = getDCRValue(doc, LANG_PATH);
								statusMap = (HashMap<String, String>) updateDeleteDynamicSurveyMasterData(surveyId,
										lang, postgre.getConnection());
							} else {
								logger.debug("Deleted status update skipped - Not Polls or Survey DCR");
							}
						} else {
					...
					if (dcrType != null) {
						if (POLLS_CONTENT_TYPE.equalsIgnoreCase(dcrType)) {
							...
							statusMap = (HashMap<String, String>) blog.processBlogDCR(taskSimpleFile, postgre);
						} else if (ERROR_CONTENT_TYPE.equalsIgnoreCase(dcrType)) {
							ErrorTask error = new ErrorTask();
							statusMap = (HashMap<String, String>) error.processErrorDCR(taskSimpleFile, postgre);
						} else if (DYNAMIC_SURVEY_CONTENT_TYPE.equalsIgnoreCase(dcrType)) {
							setOptionFieldTypes();
							statusMap = (HashMap<String, String>) processDynamicSurveyDCR(taskSimpleFile);
						} else {
							logger.info("Master data insert skipped - Not Polls or Survey DCR");
						}
					} else {
						logger.info("PollSurveyTask - DCR Type metadata is null");
					}
		...
		logger.info("transitionComment : " + statusMap.get(TRANSITION_COMMENT));
	...
	private CSSimpleFile getDeletedFile(CSClient client, CSHoleImpl taskHoleFile) {
		...
		logger.info("PollSurveyTask - getDeletedFile and client:" + client);
		try {
			logger.debug("Task file kind : " + taskHoleFile.getKind());
			if (taskHoleFile.getKind() == CSHole.KIND) {
				CSHole hole = taskHoleFile;
	...
	public Map<String, String> processPollDCR(CSSimpleFile taskSimpleFile) {
		...
		try {
			...
			if (isPollMasterDataAvailable(document)) {
				...
				logger.info("isPollDataUpdated : " + isDBOperationSuccess);
				...
				logger.info("isPollDataInserted : " + isDBOperationSuccess);
				if (isDBOperationSuccess) {
					statusMap.put(TRANSITION, SUCCESS_TRANSITION);
	...
	public Map<String, String> processSurveyDCR(CSSimpleFile taskSimpleFile) {
		...
		try {
			...
			if (isSurveyMasterDataAvailable(document)) {
				...
				logger.info("isSurveyDataUpdated : " + isDBOperationSuccess);
				if (isDBOperationSuccess) {
					statusMap.put(TRANSITION, SUCCESS_TRANSITION);
				...
				logger.info("isSurveyDataInserted : " + isDBOperationSuccess);
				if (isDBOperationSuccess) {
					statusMap.put(TRANSITION, SUCCESS_TRANSITION);
	...
	/**
	 * Method process the dynamic survey dcr from the workflow task and insert
	 * dynamic survey master data
	 *
	 * @param taskSimpleFile Task file of CSSimpleFile object
	 * @return Returns map contains the transition status and transition comment.
	 */
	public Map<String, String> processDynamicSurveyDCR(CSSimpleFile taskSimpleFile) {
		...
		try {
			...
			if (isDynamicSurveyMasterDataAvailable(document)) {
				...
				logger.info("isDynamicSurveyDataUpdated : " + isDBOperationSuccess);
				if (isDBOperationSuccess) {
					statusMap.put(TRANSITION, SUCCESS_TRANSITION);
					statusMap.put(TRANSITION_COMMENT, SURVEY_UPDATE_SUCCESS);
				} else {
	...
	public boolean isPollMasterDataAvailable(Document document) {
		...
		try {
			...
			logger.debug("pollMasterQuery in isPollMasterDataAvailable: " + pollMasterQuery);
	...
	public boolean insertPollData(Document document, String fileName) {
		...
		try {
			...
			try {
				...
				logger.error("Exception in insertPollData rollback catch block : ", ex);
			}
		} finally {
	 ...
	 * @throws SQLException
	 */
	public int insertPollMasterData(TSPollsBO pollsBO, Connection connection) throws SQLException {
		...
		try {
			...
			String pollMasterQuery = "INSERT INTO POLL_MASTER (POLL_ID, "
					...
					+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			logger.debug("insertPollMasterData pollMasterQuery : " + pollMasterQuery);
	 ...
	 * @throws SQLException
	 */
	public boolean insertPollOptionsData(TSPollsBO pollsBO, Document document, Connection connection)
			throws SQLException {
		PreparedStatement preparedStatement = null;
		boolean isPollOptionInserted = false;
		try {
			logger.info("PollSurveyTask : insertPollOptionsData");
			String pollOptionQuery = "INSERT INTO POLL_OPTION" + " (OPTION_ID, LANG, POLL_ID, OPTION_LABEL, "
					+ "OPTION_VALUE) VALUES(?, ?, ?, ?, ?)";
			preparedStatement = connection.prepareStatement(pollOptionQuery);
			logger.debug("insertPollOptionsData pollOptionQuery : " + pollOptionQuery);

			List<Node> nodes = document.selectNodes(OPTION_PATH);
			long optionId = 1l;
			for (Node node : nodes) {
				logger.debug("insertPollOptionsData optionId : " + optionId);
		...
		try {
			...
			if (result > 0) {
				...
				if (isPollOptionUpdated) {
					...
					connection.rollback();
					logger.info("updatePollData Option batch update failed");
				}
			} else {
				connection.rollback();
			...
			try {
				...
				logger.error("Exception in updatePollData rollback catch block : ", ex);
			}
		} finally {
			postgre.releaseConnection(connection, null, null);
	 ...
	 * @throws SQLException
	 */
	public int updatePollMasterData(TSPollsBO pollsBO, Connection connection) throws SQLException {
		PreparedStatement preparedStatement = null;
		int result = 0;
		try {
			...
			logger.debug("updatePollMasterData pollMasterQuery : " + pollMasterQuery);
			connection.setAutoCommit(false);
			preparedStatement = connection.prepareStatement(pollMasterQuery);
			...
			preparedStatement.setLong(8, Long.parseLong(pollsBO.getPollId()));
	...
	public Map<String, String> updateDeletePollMasterData(String pollId, String lang, Connection connection)
		...
		try {
			...
			String pollMasterQuery = "UPDATE POLL_MASTER SET STATUS = ? " + "WHERE POLL_ID = ? AND LANG = ?";
			logger.debug("updatePollMasterData pollMasterQuery : " + pollMasterQuery);
			connection.setAutoCommit(false);
			preparedStatement = connection.prepareStatement(pollMasterQuery);
			preparedStatement.setString(1, UNPUBLISHED_STATUS);
			preparedStatement.setLong(2, Long.parseLong(pollId));
	...
	public Map<String, String> updateDeleteSurveyMasterData(String surveyId, String lang, Connection connection)
		...
		try {
			...
			String surveyMasterQuery = "UPDATE SURVEY_MASTER SET STATUS = ? " + WHERE_CLAUSE_SURVEY;
			logger.debug("updateDeleteSurveyMasterData surveyMasterQuery : " + surveyMasterQuery);
			connection.setAutoCommit(false);
			preparedStatement = connection.prepareStatement(surveyMasterQuery);
			preparedStatement.setString(1, UNPUBLISHED_STATUS);
			preparedStatement.setLong(2, Long.parseLong(surveyId));
			preparedStatement.setString(3, lang);
	...
	public Map<String, String> updateDeleteDynamicSurveyMasterData(String surveyId, String lang, Connection connection)
		...
		try {
			...
			String surveyMasterQuery = "UPDATE DYNAMIC_SURVEY_MASTER SET STATUS = ? "
					+ WHERE_CLAUSE_SURVEY;
			logger.debug("updateDeleteDynamicSurveyMasterData dynamicSurveyMasterQuery : " + surveyMasterQuery);
			connection.setAutoCommit(false);
			preparedStatement = connection.prepareStatement(surveyMasterQuery);
			preparedStatement.setString(1, UNPUBLISHED_STATUS);
			preparedStatement.setLong(2, Long.parseLong(surveyId));
	 ...
	 * @throws SQLException
	 */
	public boolean updatePollOptionsData(TSPollsBO pollsBO, Document document, Connection connection)
			throws SQLException {
		PreparedStatement preparedStatement = null;
		boolean isPollOptionUpdated = false;
		try {
			logger.info("PollSurveyTask : updatePollOptionsData");
			String pollOptionQuery = "UPDATE POLL_OPTION SET " + "OPTION_LABEL = ? WHERE OPTION_ID = ? AND "
					+ "POLL_ID = ? AND LANG = ?";
			preparedStatement = connection.prepareStatement(pollOptionQuery);
			logger.debug("updatePollMasterData pollOptionQuery : " + pollOptionQuery);
	...
	public boolean isSurveyMasterDataAvailable(Document document) {
		...
		try {
			...
			logger.debug("surveyMasterQuery isSurveyMasterDataAvailable : " + surveyMasterQuery);
	 ...
	 *
	 * @param document Document object of the survey DCR
	 * @return Returns true if the dynamic survey data is already available in the
	...
	public boolean isDynamicSurveyMasterDataAvailable(Document document) {
		...
		try {
			...
			logger.debug("surveyMasterQuery isDynamicSurveyMasterDataAvailable : " + surveyMasterQuery);
			prepareStatement = connection.prepareStatement(surveyMasterQuery);
			prepareStatement.setLong(1, surveyId);
			prepareStatement.setString(2, lang);
			rs = prepareStatement.executeQuery();
	...
	public boolean insertSurveyData(Document document, String fileName) {
		...
		try {
			...
			surveyBO.setDescription(getDCRValue(document, DESCRIPTION_PATH));
			...
			logger.debug("SurveyBO : Insert :" + surveyBO);

			int result = insertSurveyMasterData(surveyBO, connection);
			logger.info("result : " + result);
			if (result > 0) {
				logger.info("Survey Master Data Inserted");
				isSurveyDataInserted = insertSurveyQuestionData(surveyBO, document, connection);
			} else {
				connection.rollback();
			...
			try {
				...
				logger.error("Exception in insertSurveyData rollback catch block : ", ex);
			}
		} finally {
			postgre.releaseConnection(connection, null, null);
	...
	public boolean insertDynamicSurveyData(Document document, String fileName) {
		...
		try {
			...
			surveyBO.setEstimatedTime(getDCRValue(document, ESTIMATED_TIME));
			surveyBO.setFileName(fileName);
			logger.debug("SurveyBO : Insert Dynamic survey :" + surveyBO);

			Long surveyMasterId = getNextSequenceValue("dynamic_survey_master_survey_master_id_seq",
					postgre.getConnection());
			...
			if (result > 0) {
				...
				isSurveyDataInserted = insertDynamicSurveyQuestionData(surveyBO, document, connection);
			} else {
				connection.rollback();
			...
			try {
				...
				logger.error("Exception in insertDynamicSurveyData rollback catch block : ", ex);
			}
		} finally {
			postgre.releaseConnection(connection, null, null);
	 ...
	 * @throws SQLException
	 */
	public int insertSurveyMasterData(TSSurveyBO surveyBO, Connection connection) throws SQLException {
		PreparedStatement preparedStatement = null;
		int result = 0;
		try {
			...
			String surveyMasterQuery = "INSERT INTO SURVEY_MASTER ("
					...
					+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			logger.debug("insertSurveyMasterData surveyMasterQuery : " + surveyMasterQuery);
			preparedStatement = connection.prepareStatement(surveyMasterQuery);
			preparedStatement.setLong(1, Long.parseLong(surveyBO.getSurveyId()));
	 ...
	 * @throws SQLException
	 */
	public int insertDynamicSurveyMasterData(TSSurveyBO surveyBO, Connection connection) throws SQLException {
		PreparedStatement preparedStatement = null;
		int result = 0;
		try {
			...
			String surveyMasterQuery = "INSERT INTO DYNAMIC_SURVEY_MASTER ("
					...
					+ "ESTIMATED_TIME, STATUS, FILE_NAME) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			logger.debug("insertDynamicSurveyMasterData surveyMasterQuery : " + surveyMasterQuery);
			preparedStatement = connection.prepareStatement(surveyMasterQuery);
			preparedStatement.setLong(1, surveyBO.getSurveyMasterId());
			preparedStatement.setLong(2, Long.parseLong(surveyBO.getSurveyId()));
	 ...
	 * @throws SQLException
	 */
	public boolean insertSurveyQuestionData(TSSurveyBO surveyBO, Document document, Connection connection)
			throws SQLException {
		PreparedStatement preparedStatement = null;
		boolean result = false;
		try {
			logger.info("PollSurveyTask : insertSurveyQuestionData");
			String surveyQuestionQuery = "INSERT INTO SURVEY_QUESTION " + "(QUESTION_ID, SURVEY_ID, LANG, QUESTION_NO, "
					+ "QUESTION_TYPE, QUESTION) VALUES (?, ?, ?, ?, ?, ?)";
			preparedStatement = connection.prepareStatement(surveyQuestionQuery);
			logger.debug("insertSurveyQuestionData surveyQuestionQuery : " + surveyQuestionQuery);

			List<Node> nodes = document.selectNodes(FIELD_PATH);
			int questionNo = 1;
			for (Node node : nodes) {
				logger.debug("insertSurveyQuestionData questionNo : " + questionNo);

				String questionType = node.selectSingleNode(FIELD_TYPE).getText();
				logger.debug("insertSurveyQuestionData questionType : " + questionType);
				...
				if (questionType != null && !"".equals(questionType) && !BUTTON.equalsIgnoreCase(questionType)) {
					...
					logger.debug("insertSurveyQuestionData questionId : " + questionId);

					String question = node.selectSingleNode(QUESTION_STR).getText();
					logger.debug("insertSurveyQuestionData question string :" + question);
					...
					if (questionResult > 0) {
						result = true;
						logger.info("insertSurveyQuestionData:Survey Question Inserted");
						if (optionsList.contains(questionType)) {
							surveyBO.setQuestionId(questionId);
							surveyBO.setQuestionNo(questionNo);
							boolean isOptionsInserted = insertSurveyOptionData(surveyBO, node, connection);
							...
							if (isOptionsInserted) {
								...
								result = false;
								connection.rollback();
								logger.info("insertSurveyQuestionData Option batch insert failed");
								break;
							}
						}
						questionNo++;
						...
						result = false;
						connection.rollback();
						logger.info("insertSurveyQuestionData Question insert failed");
						break;
					}
				}
			}
	 ...
	 * @throws SQLException
	 */
	public boolean insertDynamicSurveyQuestionData(TSSurveyBO surveyBO, Document document, Connection connection)
			throws SQLException {
		PreparedStatement preparedStatement = null;
		boolean result = false;
		try {
			...
			String surveyQuestionQuery = "INSERT INTO DYNAMIC_SURVEY_QUESTION "
					...
					+ "QUESTION_TYPE, QUESTION) VALUES (?, ?, ?, ?, ?)";
			preparedStatement = connection.prepareStatement(surveyQuestionQuery);
			logger.debug("insertDynamicSurveyQuestionData surveyQuestionQuery : " + surveyQuestionQuery);

			List<Node> nodes = document.selectNodes(OPTION_FIELD_PATH);
			for (Node node : nodes) {

				Long surveyQuestionId = getNextSequenceValue("dynamic_survey_question_survey_question_id_seq",
						postgre.getConnection());
				surveyBO.setSurveyQuestionId(surveyQuestionId);

				String questionIdStr = node.selectSingleNode(".//question-id").getText();
				if (questionIdStr != null && !"".equals(questionIdStr)) {

					long questionId = Long.parseLong(questionIdStr);
					logger.debug("insertDynamicSurveyQuestionData questionId : " + questionId);

					String questionType = node.selectSingleNode(FIELD_TYPE).getText();
					logger.debug("insertDynamicSurveyQuestionData questionType : " + questionType);
					...
					if (questionType != null && !"".equals(questionType) && !BUTTON.equalsIgnoreCase(questionType)) {
						...
						if (questionResult > 0) {
							result = true;
							logger.info("insertDynamicSurveyQuestionData :Survey Question Inserted");
							if (optionsList.contains(questionType)) {
								boolean isOptionsInserted = insertDynamicSurveyOptionData(surveyBO, node, connection);
								...
								if (isOptionsInserted) {
									...
									result = false;
									connection.rollback();
									logger.info("insertDynamicSurveyQuestionData Option batch insert failed");
									break;
								}
							}
						} else {
							result = false;
							connection.rollback();
							logger.info("insertDynamicSurveyQuestionData Question insert failed");
							break;
						}
					}
	 ...
	 * @throws SQLException
	 */
	public boolean insertSurveyOptionData(TSSurveyBO surveyBO, Node node, Connection connection) throws SQLException {
		PreparedStatement preparedStatement = null;
		boolean result = false;
		try {
			logger.info("PollSurveyTask : insertSurveyOptionData");
			String surveyOptionQuery = "INSERT INTO " + "SURVEY_OPTION (OPTION_ID, SURVEY_ID, "
					+ "LANG, QUESTION_ID, QUESTION_NO, " + "OPTION_NO, OPTION_LABEL, IS_USER_INPUT, OPTION_VALUE) "
					+ "VALUES " + "(nextval('survey_option_option_id_seq')" + ", ?, ?, ?, ?, ?, ?, ?, ?)";
			preparedStatement = connection.prepareStatement(surveyOptionQuery);
			logger.debug("surveyOptionQuery : " + surveyOptionQuery);
			List<Node> optionNodes = node.selectNodes(OPTION_STR);
			int optionNo = 1;
			for (Node optnode : optionNodes) {
				...
				String isOpenResponse = optnode.selectSingleNode(IS_OPEN_RESPONSE).getText();
			...
			int[] optionBatch = preparedStatement.executeBatch();
			logger.info("insertSurveyOptionData optionBatch length : " + optionBatch.length);

			if (optionBatch.length == optionNo - 1) {
				logger.info("success : Survey Option Inserted");
				result = true;
			}
	 ...
	 * @throws SQLException
	 */
	public boolean insertDynamicSurveyOptionData(TSSurveyBO surveyBO, Node node, Connection connection)
			throws SQLException {
		PreparedStatement preparedStatement = null;
		boolean result = false;
		try {
			...
			String surveyOptionQuery = "INSERT INTO DYNAMIC_SURVEY_OPTION "
					+ " (SURVEY_QUESTION_ID, OPTION_LABEL, IS_USER_INPUT, " + "OPTION_VALUE) VALUES (?, ?, ?, ?) ";
			preparedStatement = connection.prepareStatement(surveyOptionQuery);
			logger.debug("surveyOptionQuery : " + surveyOptionQuery);
			List<Node> optionNodes = node.selectNodes(OPTION_STR);
			int optionNo = 1;
			...
			int[] optionBatch = preparedStatement.executeBatch();
			logger.info("insertDynamicSurveyOptionData optionBatch length : " + optionBatch.length);

			if (optionBatch.length == optionNo - 1) {
				logger.info("Insert method Survey Option Inserted");
				result = true;
			}
	...
	public boolean updateSurveyData(Document document, String fileName) {
		...
		try {
			...
			surveyBO.setEstimatedTime(getDCRValue(document, ESTIMATED_TIME));
			surveyBO.setFileName(fileName);
			logger.debug("SurveyBO : update survey :" + surveyBO);
	...
	public boolean updateDynamicSurveyData(Document document, String fileName) {
		...
		try {
			...
			logger.debug("SurveyBO : update Dynamic survey :" + surveyBO);
	 ...
	 * @throws SQLException
	 */
	public void setSurveyMasterId(TSSurveyBO surveyBO, Connection connection) throws SQLException {
		PreparedStatement preparedStatement = null;
		ResultSet rs = null;
		int result = 0;
		try {
			...
			logger.debug("setSurveyMasterId dynamicSurveyMasterQuery : " + dynamicSurveyMasterQuery);
			preparedStatement = connection.prepareStatement(dynamicSurveyMasterQuery);
			preparedStatement.setLong(1, Long.parseLong(surveyBO.getSurveyId()));
			preparedStatement.setString(2, surveyBO.getLang());
	 ...
	 * @throws SQLException
	 */
	public void setSurveyQuestionId(TSSurveyBO surveyBO, Connection connection) throws SQLException {
		PreparedStatement preparedStatement = null;
		ResultSet rs = null;
		int result = 0;
		try {
			...
			logger.debug("setSurveyQuestionId dynamicSurveyMasterQuery : " + dynamicSurveyMasterQuery);
			preparedStatement = connection.prepareStatement(dynamicSurveyMasterQuery);
			preparedStatement.setLong(1, surveyBO.getSurveyMasterId());
	 ...
	 * @throws SQLException
	 */
	public int updateSurveyMasterData(TSSurveyBO surveyBO, Connection connection) throws SQLException {
		PreparedStatement preparedStatement = null;
		int result = 0;
		try {
			logger.info("PollSurveyTask : updateSurveyMasterData");
			String surveyMasterQuery = "UPDATE SURVEY_MASTER SET " + "SURVEY_TITLE = ?, SURVEY_DESCRIPTION = ?, "
					+ "END_DATE = ?, PERSONA = ?, SERVICE_ENTITIES = ?, "
					+ "TOPICS = ?, SUBMIT_TYPE = ?, STATUS = ? , FILE_NAME = ?, ESTIMATED_TIME = ? "
					+ WHERE_CLAUSE_SURVEY;
			logger.debug("updateSurveyMasterData surveyMasterQuery : " + surveyMasterQuery);
			preparedStatement = connection.prepareStatement(surveyMasterQuery);
			...
			preparedStatement.setLong(11, Long.parseLong(surveyBO.getSurveyId()));
	 ...
	 * @throws SQLException
	 */
	public int updateDynamicSurveyMasterData(TSSurveyBO surveyBO, Connection connection) throws SQLException {
		PreparedStatement preparedStatement = null;
		int result = 0;
		try {
	 ...
	 * @throws SQLException
	 */
	public boolean updateSurveyQuestionData(TSSurveyBO surveyBO, Document document, Connection connection)
			throws SQLException {
		PreparedStatement preparedStatement = null;
		boolean result = false;
		try {
			...
			String surveyQuestionQuery = "UPDATE SURVEY_QUESTION SET "
					+ "QUESTION = ? WHERE SURVEY_ID = ? AND LANG = ? " + "AND QUESTION_NO = ?";
			preparedStatement = connection.prepareStatement(surveyQuestionQuery);
			...
			int questionNo = 1;
			for (Node node : nodes) {
				logger.debug("updateSurveyQuestionData questionNo : " + questionNo);

				String questionType = node.selectSingleNode(FIELD_TYPE).getText();
				logger.debug("updateSurveyQuestionData questionType : " + questionType);

				if (questionType != null && !"".equals(questionType) && !BUTTON.equalsIgnoreCase(questionType)) {

					String question = node.selectSingleNode(QUESTION_STR).getText();
					logger.debug("question value: " + question);
					...
					if (questionResult > 0) {
						result = true;
						logger.info("updateSurveyQuestionData:Survey Question Inserted");
						if (optionsList.contains(questionType)) {
							surveyBO.setQuestionNo(questionNo);
							boolean isOptionsUpdated = updateSurveyOptionData(surveyBO, node, connection);
							...
							if (isOptionsUpdated) {
								...
								result = false;
								connection.rollback();
								logger.info("updateSurveyQuestionData Option batch update failed");
								break;
							}
							questionNo++;
						}
						...
						result = false;
						connection.rollback();
						logger.info("updateSurveyQuestionData Question update failed");
						break;
					}
				}
			}
	 ...
	 * @throws SQLException
	 */
	public boolean updateDynamicSurveyQuestionData(TSSurveyBO surveyBO, Document document, Connection connection)
			throws SQLException {
		PreparedStatement preparedStatement = null;
		boolean result = false;
		try {
			...
			for (Node node : nodes) {
				...
				String questionType = node.selectSingleNode(FIELD_TYPE).getText();
				logger.debug("updateDynamicSurveyQuestionData questionType : " + questionType);
				...
				if (questionType != null && !"".equals(questionType) && !BUTTON.equalsIgnoreCase(questionType)) {
					...
					if (questionResult > 0) {
						result = true;
						logger.info("updateDynamicSurveyQuestionData:Survey Question Inserted");
						if (optionsList.contains(questionType)) {
							boolean isOptionsUpdated = updateDynamicSurveyOptionData(surveyBO, node, connection);
							...
							if (isOptionsUpdated) {
								...
								result = false;
								connection.rollback();
								logger.info("updateDynamicSurveyQuestionData Option batch update failed");
								break;
							}
						}
					} else {
						result = false;
						connection.rollback();
						logger.info("updateDynamicSurveyQuestionData Question update failed");
						break;
					}
				}
			}
	 ...
	 * @throws SQLException
	 */
	public boolean updateSurveyOptionData(TSSurveyBO surveyBO, Node node, Connection connection) throws SQLException {
		PreparedStatement preparedStatement = null;
		boolean result = false;
		try {
			logger.info("PollSurveyTask : updateSurveyOptionData");
			String surveyOptionQuery = "UPDATE " + "SURVEY_OPTION SET OPTION_LABEL = ?,  IS_USER_INPUT = ? "
					+ "WHERE SURVEY_ID = ? " + "AND LANG = ? AND QUESTION_NO = ? " + "AND OPTION_NO = ? ";
			preparedStatement = connection.prepareStatement(surveyOptionQuery);
			logger.debug("updateSurveyOptionData surveyOptionQuery : " + surveyOptionQuery);
			List<Node> optionNodes = node.selectNodes(OPTION_STR);
			int optionNo = 1;
			for (Node optnode : optionNodes) {
				logger.debug("updateSurveyOptionData optionNo : " + optionNo);
				String optionLabel = optnode.selectSingleNode(OPTION_LABEL).getText();
				String isOpenResponse = optnode.selectSingleNode(IS_OPEN_RESPONSE).getText();
				String optionValue = optnode.selectSingleNode(OPTION_VALUE).getText();
				logger.debug(optionLabel + " : " + optionValue);
				...
				preparedStatement.setLong(3, Long.parseLong(surveyBO.getSurveyId()));
			...
			int[] optionBatch = preparedStatement.executeBatch();
			logger.info("updateSurveyOptionData optionBatch length : " + optionBatch.length);

			if (optionBatch.length == optionNo - 1) {
				logger.info("Update mehod Survey Option Inserted");
				result = true;
			}
	 ...
	 * @throws SQLException
	 */
	public boolean updateDynamicSurveyOptionData(TSSurveyBO surveyBO, Node node, Connection connection)
			throws SQLException {
		PreparedStatement preparedStatement = null;
		boolean result = false;
		try {
			logger.info("PollSurveyTask : updateDynamicSurveyOptionData");
			String surveyOptionQuery = "UPDATE " + "DYNAMIC_SURVEY_OPTION SET OPTION_LABEL = ?,  IS_USER_INPUT = ? "
					+ "WHERE SURVEY_QUESTION_ID = ? ";
			preparedStatement = connection.prepareStatement(surveyOptionQuery);
			logger.debug("updateDynamicSurveyOptionData surveyOptionQuery : " + surveyOptionQuery);
			List<Node> optionNodes = node.selectNodes(OPTION_STR);
			int optionNo = 1;
			for (Node optnode : optionNodes) {
				logger.debug("updateDynamicSurveyOptionData optionNo : " + optionNo);
				String optionLabel = optnode.selectSingleNode(OPTION_LABEL).getText();
				String isOpenResponse = optnode.selectSingleNode(IS_OPEN_RESPONSE).getText();
			...
			int[] optionBatch = preparedStatement.executeBatch();
			logger.info("updateDynamicSurveyOptionData optionBatch length : " + optionBatch.length);

			if (optionBatch.length == optionNo - 1) {
				logger.info("updateDynamicSurveyOptionData :Survey Option Inserted");
				result = true;
			}
	 ...
	 */
	private Long getNextSequenceValue(String sequenceName, Connection connection) {
		Long seqValue = 0L;
		PreparedStatement queryStmt = null;
		ResultSet rs = null;
		try {
			String query = "SELECT nextval('" + sequenceName + "') as seqValue";
	...
	/**
	 * Method post process the workflow inputs
	 *
	...
	public void postProcessWorkFlowInputs(CSFile file, CSExternalTask taskObj) throws CSException {
		...
		try {
			...
			logger.error("Exception in postProcessWorkFlowInputs: ", e);
		}
	}
