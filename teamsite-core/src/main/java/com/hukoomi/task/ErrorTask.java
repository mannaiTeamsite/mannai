...
public class ErrorTask {
	 ...
	 */
	public Map<String, String> processErrorDCR(CSSimpleFile taskSimpleFile, PostgreTSConnection postgre) {
		...
		try {
			...
			if (isErrorMasterDataAvailable(document, postgre)) {
				...
				logger.info("isErrorDataUpdated : " + isDBOperationSuccess);
				if (result > 0) {
				...
				logger.info("isErrorDataInserted : " + isDBOperationSuccess);
	...
	private boolean insertErrorData(Document document, PostgreTSConnection postgre) {
		...
		try {
			...
			logger.error("Exception in insertErrorData rollback catch block : ", e);
		} finally {
	...
	private int insertErrorMasterData(ErrorBO errorBO, Connection connection, PostgreTSConnection postgre) {
		PreparedStatement preparedStatement = null;
		int result = 0;
		try {
			logger.info("ErrorTask : insertBolgMasterData");

			String errorMasterQuery = "INSERT INTO ERROR_MASTER (" + " STATUS_CODE, LANGUAGE, ERROR_NAME_TECHNICAL,"
					+ "ERROR_TITLE, ERROR_MESSAGE ) " + "VALUES ( ?, ?, ?, ?, ?)";
			logger.info("insertErrorMasterData errorMasterQuery : " + errorMasterQuery);
			preparedStatement = connection.prepareStatement(errorMasterQuery);
	...
	private int updateErrorData(Document document, PostgreTSConnection postgre) {
		PreparedStatement preparedStatement = null;
		int result = 0;
		Connection connection = null;
		...
		try {
			...
			logger.info("updateErrorMasterData Erorr master query : " + errorMasterQuery);
	...
	private boolean isErrorMasterDataAvailable(Document document, PostgreTSConnection postgre) {
		...
		try {
			...
			logger.info("pollMasterQuery in isErrorMasterDataAvailable: " + errorMasterQuery);
			prepareStatement = connection.prepareStatement(errorMasterQuery);
			...
			logger.info("Released connection in isErrorDataAvailable");
		}
